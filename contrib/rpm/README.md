...
reference the release tarball available from https://zurcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Bitcoin git repository](https://github.com/zurcoin/zurcoin) but are not part of
...
Currently only `Source10` is used, for the example `zurcoin.conf` file.
...
in the zurcoin git repository. At present only an SVG version of the Bitcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the zurcoin
...
The `zurcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `zurcoin-server` package. At this point, most RPM based
...
The `zurcoin-server` package should have SELinux support. How to properly do
...
[Ringing Liberty](https://www.ringingliberty.com/zurcoin/). He has been
...
4. I renamed the `zurcoin` package that contains the Qt GUI to `zurcoin-core` as
that appears to be how the general population refers to it, in contrast to
`zurcoin-xt` or `zurcoin-classic`. I wanted to make sure the general population
