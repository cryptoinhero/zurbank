...
#include "zurcoin-config.h" /* for USE_QRCODE */
#endif
...
void LookupAddressDialog::searchAddress()
{
    ...
    // lets see if the string is a valid zurcoin address
    ...
    if (address.IsValid()) //do what?
    {
        ...
        ui->balanceLabel->setText(QString::fromStdString(FormatDivisibleMP(GetAvailableTokenBalance(searchText, 1)) + " ZUS"));
        // QR
        #ifdef USE_QRCODE
        ui->QRCode->setText("");
        ...
        for (pItem = 1; pItem < 11; pItem++)
        {
            if (pFound[pItem])
            {
                ...
                if (pName[pItem]=="Test Zurshares tokens (#2)") { tokenLabel = " TZUS"; }
                if (pDivisible[pItem])
                {
