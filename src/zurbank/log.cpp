...
// Default log files
const std::string LOG_FILENAME    = "zurbank.log";
 ...
 *
 * The log file can be specified via startup option "--omnilogfile=/path/to/zurbank.log",
...
int LogFilePrint(const std::string& str)
{
    ...
    if (fPrintToConsole) {
        ...
        if (fReopenZurBankLog) {
            fReopenZurBankLog = false;
            boost::filesystem::path pathDebug = GetLogPath();
