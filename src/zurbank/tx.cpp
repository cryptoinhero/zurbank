...
std::string mastercore::strTransactionType(uint16_t txType)
{
    switch (txType) {
        ...
        case MSC_TYPE_ACCEPT_OFFER_ZUR: return "DEx Accept Offer";
...
bool CMPTransaction::interpret_Transaction()
{
    ...
    switch (type) {
        ...
        case MSC_TYPE_ACCEPT_OFFER_ZUR:
            return interpret_AcceptOfferZUR();
...
/** Tx 22 */
bool CMPTransaction::interpret_AcceptOfferZUR()
{
    if (pkt_size < 16) {
        return false;
    }
...
int CMPTransaction::interpretPacket()
{
    ...
    switch (type) {
        ...
        case MSC_TYPE_ACCEPT_OFFER_ZUR:
            return logicMath_AcceptOffer_ZUR();
...
int CMPTransaction::logicMath_TradeOffer()
{
    ...
    if (OMNI_PROPERTY_TMSC != property && OMNI_PROPERTY_MSC != property) {
        PrintToLog("%s(): rejected: property for sale %d must be ZUS or TZUS\n", __func__, property);
...
/** Tx 22 */
int CMPTransaction::logicMath_AcceptOffer_ZUR()
{
    if (!IsTransactionTypeAllowed(block, property, type, version)) {
        PrintToLog("%s(): rejected: type %d or version %d not permitted for property %d at block %d\n",
                __func__,
...
int CMPTransaction::logicMath_MetaDExTrade()
{
    ...
    if (!IsFeatureActivated(FEATURE_TRADEALLPAIRS, block)) {
        ...
        if ((property != OMNI_PROPERTY_MSC) && (desired_property != OMNI_PROPERTY_MSC) &&
            ...
            PrintToLog("%s(): rejected: one side of a trade [%d, %d] must be ZUS or TZUS\n", __func__, property, desired_property);
