...
void RequirePrimaryToken(uint32_t propertyId)
{
    if (propertyId < 1 || 2 < propertyId) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Property identifier must be 1 (ZUS) or 2 (TZUS)");
    }
}
...
{
    if ((0.01 * COIN) < amount) {
        throw JSONRPCError(RPC_TYPE_ERROR, "Reference amount higher is than 0.01 ZUR");
    }
}
...
void RequireSaneDExFee(const std::string& address, uint32_t propertyId)
{
    ...
    if (poffer->getMinFee() > 1000000) {
        throw JSONRPCError(RPC_TYPE_ERROR, "Minimum accept fee is higher than 0.01 ZUR (use override = true to continue)");
    }
}
