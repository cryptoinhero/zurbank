...
- [Building Bitcoin Core](#building-zurcoin-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-zurcoin-host/README.md).
...
Clone the git repositories for zurcoin and Gitian.
...
in the zurcoin repository under 'Fetch and create inputs' to install sources which require
...
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the zurcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/zurcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/zurcoin/zurcoin
...
For example:
```bash
URL=https://github.com/laanwj/zurcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit zurcoin=${COMMIT} --url zurcoin=${URL} ../zurcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit zurcoin=${COMMIT} --url zurcoin=${URL} ../zurcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit zurcoin=${COMMIT} --url zurcoin=${URL} ../zurcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the zurcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../zurcoin/contrib/gitian-descriptors/*|sort|uniq )
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/zurcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/zurcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/zurcoin-osx-build.assert
```
...
[zurcoin-core/gitian.sigs](https://github.com/zurcoin-core/gitian.sigs/) repository, or if that's not possible create a pull
